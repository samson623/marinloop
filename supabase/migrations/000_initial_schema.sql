-- Bootstrap migration: applies the base schema to a fresh database.
-- All statements are idempotent — safe to run on production where the schema already exists.
-- Types are wrapped in DO/EXCEPTION blocks; tables, indexes, functions, triggers use IF NOT EXISTS / CREATE OR REPLACE.

create extension if not exists "uuid-ossp";

DO $$ BEGIN CREATE TYPE public.plan_type AS ENUM ('free', 'pro', 'family'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE public.dose_status AS ENUM ('taken', 'late', 'missed', 'skipped'); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE TYPE public.notification_type AS ENUM ('info', 'warning', 'success', 'error'); EXCEPTION WHEN duplicate_object THEN null; END $$;
...
create policy dose_logs_select_own on public.dose_logs for select using (auth.uid() = user_id);
drop policy if exists dose_logs_insert_own on public.dose_logs;
create policy dose_logs_insert_own on public.dose_logs for insert with check (auth.uid() = user_id);
drop policy if exists dose_logs_update_own on public.dose_logs;
create policy dose_logs_update_own on public.dose_logs for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists dose_logs_delete_own on public.dose_logs;
create policy dose_logs_delete_own on public.dose_logs for delete using (auth.uid() = user_id);
...
create policy refills_select_own on public.refills for select using (auth.uid() = user_id);
drop policy if exists refills_insert_own on public.refills;
create policy refills_insert_own on public.refills for insert with check (auth.uid() = user_id);
drop policy if exists refills_update_own on public.refills;
create policy refills_update_own on public.refills for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists refills_delete_own on public.refills;
create policy refills_delete_own on public.refills for delete using (auth.uid() = user_id);
